# Content SV Hellern

## Quellen

- `Claud-Projekt/CHECKLISTE.md`
- `DOC/Claude-ErstkontaktVerein.txt`
- `UPLOUD/homepage_vorschau.html`
- bestehende Website: https://www.sv-hellern.com/

## Bekannte Inhalte

- umfangreiche Mehrspartenstruktur
- Praesidium / Vorstand 2026
- Geschaeftsstelle mit Oeffnungszeiten
- Abteilungs-Ansprechpartner
- News-Artikel, mindestens letzte 12 Monate geplant
- Termine / Kalender
- Downloads / PDFs
- Archivseite
- Kontaktformular
- Datenschutz / Cookie-Consent
- Ligawidgets fuer Fussball
- oeffentlich sichtbare Bereiche: Aktuell, Der Verein, Abteilungen, 100 Jahre SVH, SVH-Clubhouse, Download, Hallenplaene, Kontakt, Impressum

## Abteilungsinhalte

Zu migrieren laut Checkliste:

- Fussball
- Basketball
- Volleyball
- Handball
- Tennis / Tischtennis / Badminton
- Radsport
- Schach
- Turnen / Tanzen / Bewegung
- Fitness & Gesundheit
- Rehasport
- Ballschule

Zusaetzlich oeffentlich sichtbar:

- Tamburello

## Regel

Die Abteilungsstruktur ist verbindlich zu pruefen und nicht eigenstaendig zu kuerzen. Da kein Datenexport und keine Zugangsdaten vorliegen, muessen Inhalte aus der Altdomain noch gesichert und gegen den CMS-Stand abgeglichen werden.
